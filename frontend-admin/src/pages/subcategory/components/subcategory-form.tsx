...
const SubcategoryForm: React.FC<SubcategoryFormProps> = ({
  ...
  async function onDelete() {
    if (initialData) {
      ...
      const response = await fetch(
        `http://localhost:4000/api/subcategory/${initialData.id}`,
        {
          method: "DELETE",
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (initialData) {
      ...
      response = await fetch("http://localhost:4000/api/subcategory/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...values, products: [] }),
      });
    ...
    if (response.ok) {
      ...
      navigate("/subcategory", { replace: true });
      toast.success(
        initialData ? "Subcategory updated." : "Subcategory created.",
      );
    }
